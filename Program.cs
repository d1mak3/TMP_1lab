...
namespace TMP_1lab
{
    class Program
    {
        ...
        static int MinInQueue(Queue _queue) // Максимальное число в очереди
        {
            Queue tempqueue = new Queue();
            int temp = 0;
        ...
        static void Main()
        {
			...
			{
                tmp.Push(randomNumber.Next(Convert.ToInt32(input[1]), Convert.ToInt32(input[2]))); // Переводим в int интервал (извиняюсь за "весь код в одну строчку" XD)
			}
